...
public class RabinKarpTest {
	...
	public int strStr(String haystack, String needle) {
        ...
        int start = 0, currentCode = 0;
        do{
            currentCode = (start == 0) ? 
            		hashcode(haystack.substring(start, len)) : 
            		nextCode(start-1, haystack, len, currentCode);
            if(patternCode == currentCode && needle.equals(haystack.substring(start,start+len))) {
                return start;
            }
            start++;
